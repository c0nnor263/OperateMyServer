...
class EmptyServerFeatureTest : ShouldSpec({

    lateinit var sut: EmptyServerFeature
    val mockConfig = mockk<CEmptyServerFeature>(relaxed = true)
    val mockConfigProvider: ConfigProvider<CEmptyServerFeature> = mockk()
    ...
    context("info") {
        ...
        should("override id and priority, but preserve data map") {
            val a = sut.info()
            a.id shouldBe CEmptyServerFeature.NAME
            a.priority shouldBe Priority.COMMON

            val b = sut.info()
            b.id shouldBe CEmptyServerFeature.NAME
    ...
    context("createEventListeners") {
        should("return list with EmptyServerEvents") {
            val listeners = sut.createEventListeners()
            listeners.size shouldBe 1
            listeners[0].shouldBeInstanceOf<EmptyServerEvents>()
        }
    }
})
