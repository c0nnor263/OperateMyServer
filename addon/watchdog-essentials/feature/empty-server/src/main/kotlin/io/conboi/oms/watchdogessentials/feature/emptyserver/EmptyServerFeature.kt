...
class EmptyServerFeature(
    ...
    val countTime: CachedField<String, Duration> = configField {
    ...
    val shouldRestart: Boolean by configField {
        key = { config.shouldRestart.get() }
        value = { config.shouldRestart.get() }
    }
    ...
    override fun onOmsTick(event: OMSLifecycle.TickingEvent, context: AddonContext) {
        ...
        if (elapsedSec >= countTime.get().inWholeSeconds) {
            ...
            FORGE_BUS.post(OMSActions.StopRequestedEvent(server, EmptyServerStop(shouldRestart = shouldRestart)))
            clearTime()
        }
    }
