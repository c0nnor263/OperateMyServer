...
object OMSPersistentLoggerService {

    private val cache = mutableMapOf<String, Logger>()

    fun createPersistentLogger(
        modId: String,
        featureId: String,
        logDir: Path
    ): OMSPersistentFileLogger {
        val name = "$modId.$featureId"

        val logger = cache.getOrPut(name) {
            createLogger(name, logDir)
        }

        return OMSPersistentFileLogger(logger)
    }


    private fun createLogger(name: String, logDir: Path): Logger {
        Files.createDirectories(logDir)

        val filePath = logDir.resolve("$name.log").toString()
