...
class FeatureManagerTest : ShouldSpec({
    ...
    context("onStartingEvent") {
        ...
        should("call onOmsStarted on all features") {
            ...
            sut.onStartingEvent(event)

            verify { mockFeature1.onOmsStarted(event) }
            verify { mockFeature2.onOmsStarted(event) }
        }
    }
    ...
    context("onTickingEvent") {
        ...
        should("tick only enabled features") {
            ...
            sut.onTickingEvent(event)

            verify(exactly = 0) { mockFeature1.onOmsTick(any()) }
            verify(exactly = 1) { mockFeature2.onOmsTick(event) }
        }
    ...
    context("onStoppingEvent") {
        ...
        should("call onOmsStopping for all features and clear registry") {
            ...
            sut.onStoppingEvent(event)

            verify { mockFeature1.onOmsStopping(event) }
            verify { mockFeature2.onOmsStopping(event) }
